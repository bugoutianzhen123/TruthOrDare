...
type Card interface {
	...
	GetCard(mode, ty, style int8) *[]domain.Card
}
...
func (d *dao) GetCard(mode, ty, style int8) *[]domain.Card {
	var cards []domain.Card
	d.db.Model(&cards).Where("mode = ? and type = ? and style = ?", mode, ty, style).Find(&cards)
	return &cards
}
